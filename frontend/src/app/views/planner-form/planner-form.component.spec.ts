...
describe('PlannerFormComponent', () => {
	...
	it('should form has proper controls', () => {
		...
		expect(component.formModel()?.formGroup().get(rangeTimeControl)).toBeTruthy();
		expect(component.formModel()?.formGroup().get(isConstantControl)).toBeTruthy();
	});

	it('should form has default values on form', () => {
		expect(component.formModel()?.formGroup().get(nameControl)?.value).toBe('');
		expect(
			component.formModel()?.formGroup().get(noteControl)?.value,
		).toBe('');
		expect(component.formModel()?.formGroup().get(rangeTimeControl)?.value).toBe('');
		expect(component.formModel()?.formGroup().get(isConstantControl)?.value).toBe(null);
	});
	...
	it('should control rangeTime has error, if user provide invalid value', () => {
		const rangeTime = component.formModel()?.formGroup().get(rangeTimeControl);
